...
class CustomerSalesXlsx(models.AbstractModel):
    _name = 'report.task1.report_purchase_orders_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, partner_id):
        ...
        header_data.update(
            [('Vendor', form_data.get('partner_id')[1] if form_data.get('partner_id') else None),
        ...
        for rec in sales_order_count:
            ...
            for id in order_lines:
                if rec == id['order_id'][1]:
                    r, c = 5, 0
                    sheet.write(r, c, 'Purchase Order ID', bold)
